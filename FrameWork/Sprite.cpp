...
void Sprite::DrawUV(float x, float y, int frameIndex, int totalFrameCount)
{
    if (!Texture || totalFrameCount <= 0) return;

    // 1. 프레임 너비 계산
    int frameWidth = imagesinfo.Width / totalFrameCount;
    int frameHeight = imagesinfo.Height;

    // 2. 현재 프레임의 좌상단 위치 계산
    float srcX = frameWidth * frameIndex;
    float srcY = 0;

    // 3. 기존 Draw 함수를 활용하여 출력
    this->Draw(x, y, srcX, srcY, static_cast<float>(frameWidth), static_cast<float>(frameHeight));
}
